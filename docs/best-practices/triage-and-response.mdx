...
Such failures can be:
    1. Newley configured anomaly detection tests or explicit tests where you have low certainty about the threshold / expectation.
    2. Anomaly detection tests that you consider as a safety measure, not a clear failure. 
...
![Incident management dashboard](/pics/incident-management.png)
...
![An interface showing initiating triage from incident management](/pics/triage-response-via-incident-management.png)
...
- Does the failure impact important downstream assets? Did the issue propagate to downstream assets?
    - A table might not be critical, but it’s upstream from a critical one, making it part of a critical path.
        ...
        ![Lineage filters](/pics/lineage-filters.png)
        
        - Use the lineage filters to color and highlight all the tables in a path that match your filtering criteria.
        
        ![Lineage filters showing the add filter interface](/pics/lineage-filters-2.webp)
...
- What is the magnitude of the failure? How many failed results out of the total volume?
    - Most tests return the number of failed results. A failure in a `unique` test for can be dramatic if it impacts many rows, but insignificant if there is just one case of duplicates.
        ...
        ![Graph showing test performance](/pics/test-performance-graph.png)
...
- Is there a data issue at the source?
    ...
    - *Coming soon - Check the metric graphs of the source tables.*
- Is it a code issue?
